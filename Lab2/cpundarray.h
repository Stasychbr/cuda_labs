...
class CPUNDArray : public NDArray {
   ...
   public:
    CPUNDArray(size_t ndim, const size_t* shape, const T* plain_data = nullptr);
    ...
    CPUNDArray<T> mtx_mlp(const CPUNDArray<T>& ndarray) const;
...
template <typename T>
CPUNDArray<T>::CPUNDArray(size_t ndim, const size_t* shape, const T* plain_data) {
    _ndim = ndim;
...
template <typename T>
CPUNDArray<T> CPUNDArray<T>::mtx_mlp(const CPUNDArray<T>& ndarray) const {
    ...
    CPUNDArray<T> res = CPUNDArray<T>(2, res_shape);
    T* data_1 = get_data_ptr();
    T* data_2 = ndarray.get_data_ptr();
    T* res_data = res.get_data_ptr();
    size_t idx_1 = 0; 
    size_t idx_2 = 0;
    size_t idx_w = 0;
    for (size_t i = 0; i < res_shape[0]; i++) {
        ...
        for (size_t j = 0; j < res_shape[1]; j++) {
            ...
            for (size_t k = 0; k < shape[1]; k++) {
                res_data[idx_w] += data_1[idx_1 + k] * data_2[idx_2 + k * other_shape[1]];
            }
            idx_w++;
            idx_2++;
        }
